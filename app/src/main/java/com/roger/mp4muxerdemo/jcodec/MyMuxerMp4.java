...
public class MyMuxerMp4 {
    ...
    public MyMuxerMp4(SeekableByteChannel output) throws IOException {
        this(output, Brand.MP4, true);
    }

    public MyMuxerMp4(SeekableByteChannel output, Brand brand, boolean reWrite) throws IOException {
        this(output, brand.getFileTypeBox(), reWrite);
    }

    public MyMuxerMp4(SeekableByteChannel output, FileTypeBox ftyp, boolean reWrite) throws IOException {
        this.out = output;
        if (reWrite) {
    ...
    public FramesMP4MuxerTrack2 addVideoTrackWithTimecode(String fourcc, Size size, String encoderName, int timescale) {
        TimecodeMP4MuxerTrack timecode = addTimecodeTrack(timescale);

        FramesMP4MuxerTrack2 track = addTrack(VIDEO, timescale);
    ...
    public FramesMP4MuxerTrack2 addVideoTrack(String fourcc, Size size, String encoderName, int timescale) {
        FramesMP4MuxerTrack2 track = addTrack(VIDEO, timescale);
    ...
    public static AudioSampleEntry audioSampleEntry(String fourcc, int drefId, int sampleSize, int channels,
                                                    int sampleRate, EndianBox.Endian endian) {
    ...
    public FramesMP4MuxerTrack2 addTrack(TrackType type, int timescale) {
        FramesMP4MuxerTrack2 track = new FramesMP4MuxerTrack2(out, nextTrackId++, type, timescale);
        tracks.add(track);
        return track;
    }
    ...
    public PCMMP4MuxerTrack addPCMTrack(int timescale, int sampleDuration, int sampleSize,
                                        SampleEntry se) {
        PCMMP4MuxerTrack track = new PCMMP4MuxerTrack(out, nextTrackId++, SOUND, timescale, sampleDuration, sampleSize, se);
        tracks.add(track);
        return track;
    }
    ...
    public MovieBox finalizeHeader() throws IOException {
        ...
        for (AbstractMP4MuxerTrack track : tracks) {
            try {
                Class<?> classType = track.getClass();
                Method method = classType.getDeclaredMethod("finish",
                        new Class[]{MovieHeaderBox.class});
                method.setAccessible(true); // 抑制Java的访问控制检查
                Box trak = (Box) method.invoke(track, new Object[]{mvhd});
                if (trak != null)
                    movie.add(trak);
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static AudioSampleEntry audioSampleEntry(AudioFormat format) {
        return MP4Muxer.audioSampleEntry(lookupFourcc(format), 1,
                ...
                format.isBigEndian() ? EndianBox.Endian.BIG_ENDIAN : EndianBox.Endian.LITTLE_ENDIAN);
    }

    public FramesMP4MuxerTrack2 addCompressedAudioTrack(String fourcc, int timescale, int channels, int sampleRate,
                                                        int samplesPerPkt, Box... extra) {
        FramesMP4MuxerTrack2 track = addTrack(SOUND, timescale);
