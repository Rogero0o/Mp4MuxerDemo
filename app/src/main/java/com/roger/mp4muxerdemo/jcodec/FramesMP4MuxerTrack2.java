...
public class FramesMP4MuxerTrack2 extends AbstractMP4MuxerTrack {
    ...
    private List<CompositionOffsetsBox.Entry> compositionOffsets = new ArrayList<CompositionOffsetsBox.Entry>();
    ...
    public FramesMP4MuxerTrack2(SeekableByteChannel out, int trackId, TrackType type, int timescale) {
    ...
    public void addFrame(MP4Packet pkt) throws IOException {
        ...
        ListCache.getInstance(context).saveLastIndex(curFrame);
    ...
    public void setTrackTotalDuration(long total) {
        trackTotalDuration = total - 1;
        ptsEstimate = total - 1;
        chunkNo = (int) total - 1;
        lastCompositionSamples = (int) total - 1;
        iframes.clear();
        sameDurCount = total - 1;
        samplesInLastChunk = 1;
        curDuration = 1;
        samplesInChunks = ListCache.getInstance(context).getBeanList();
        sampleSizes = ListCache.getInstance(context).getSampleSizes();
        sampleDurations = ListCache.getInstance(context).getSampleDurationsList();
        chunkOffsets = ListCache.getInstance(context).getChunkOffsets();
        iframes = ListCache.getInstance(context).getIFRAMESizes();
        allIframes = false;
    }
    ...
    protected Box finish(MovieHeaderBox mvhd) throws IOException {
        ...
        stbl.add(new SampleToChunkBox(samplesInChunks.toArray(new SampleToChunkBox.SampleToChunkEntry[0])));
        stbl.add(new SampleSizesBox(sampleSizes.toArray()));
        stbl.add(new TimeToSampleBox(sampleDurations.toArray(new TimeToSampleBox.TimeToSampleEntry[]{})));
    ...
    private void putCompositionOffsets(NodeBox stbl) {
        if (compositionOffsets.size() > 0) {
            ...
            CompositionOffsetsBox.Entry first = compositionOffsets.get(0);
            ...
            stbl.add(new CompositionOffsetsBox(compositionOffsets.toArray(new CompositionOffsetsBox.Entry[0])));
        }
    }
